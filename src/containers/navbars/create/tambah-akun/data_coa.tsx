...
export interface SelectedOption {
  ...
  onMount(async () => {
    ...
    // Membuat opsi dari data COA yang diambil
    const isoptions = data_coa.map(coa => ({
      value: coa.coa_kd,
      label: coa.coa_name,
    }));

    console.log("ini apa", isoptions)
    // Set opsi ke dalam state
    setOptions(isoptions);
  });

    const [options, setOptions] = createSignal<Option[]>([
    // { value: `${coa.coa_kd}`, label: `${coa.coa_name}` },
    ])
